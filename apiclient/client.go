// Package apiclient is a thin REST client for the studio-cp tenant API.
//
...
// Client is a tenant-scoped studio-cp REST client.
type Client struct {
	endpoint string
	token    string
	hc       *http.Client
...
func (e *APIError) Error() string {
	...
	return fmt.Sprintf("studio-cp API error %d: %s", e.Status, msg)
}
